...
void Connection::Start() {
    ...
    _event.events |= EPOLLET; //edge trigger
    ...
    try {
        ...
        while ((readed_bytes = read(_socket, client_buffer, sizeof(client_buffer))) > 0) {
            ...
            while (readed_bytes > 0) {
                ...
                if (command_to_execute && arg_remains == 0) {
                    //END COPYPAST
                    std::unique_lock<std::mutex> _lock(queue_push_mutex);
...
void Connection::DoWrite() { 
    ...
    while(!out_queue.empty()){
        ...
        out_queue.pop_front();
        head_offset = 0;
        if(out_queue.size() < max_size && (_event.events & EPOLLIN) != EPOLLIN){
            _event.events |= EPOLLIN;
        }
    }

...
void ServerImpl::SocketWorker(int client_socket){
    //START: CopyPast from st_blocking
    std::size_t arg_remains;
    Protocol::Parser parser;
    std::string argument_for_command;
    std::unique_ptr<Execute::Command> command_to_execute;
    ...
    // Cleanup on exit...
    _logger->warn("Socket_worker end socket: {}", client_socket);
}
